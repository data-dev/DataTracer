...
@dask.delayed
def evaluate_single_composite_column_map(constraint, tracer, tables):
...
@dask.delayed
def composite_column_map(solver, target, datasets):
    ...
    for constraint in metadata.data["constraints"]:
        list_of_metrics.append(evaluate_single_composite_column_map(constraint, tracer, tables))

    list_of_metrics = dask.compute(list_of_metrics)[0]
    return list_of_metrics


def benchmark_composite_column_map(data_dir, dataset_name=None, solver="datatracer.column_map_composite.basic"):
    ...
    for dataset_name in dataset_names:
        dataset_to_metrics[dataset_name] = composite_column_map(
